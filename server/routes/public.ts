...
export function registerPublicRoutes(app: Express): void {
   ...
   * ✅ DOIT être enregistrée AVANT les autres routes pour éviter l'interception
   */
  app.get("/accept/:orderId", async (req, res) => {
    try {
      ...
      if (driverId && typeof driverId === 'string') {
        ...
        const appUrl = process.env.APP_URL || "https://tataouine-pizza.onrender.com";
        const autoLoginUrl = `${appUrl}/driver/auto-login?token=${token}&driverId=${driver.id}&driverName=${encodeURIComponent(driver.name)}&driverPhone=${encodeURIComponent(driver.phone)}&order=${orderId}&accepted=true`;
        console.log("[ACCEPT] 🔄 Redirection vers auto-login (URL complète):", autoLoginUrl);
        console.log("[ACCEPT] ✅ Token généré, longueur:", token.length);
        console.log("[ACCEPT] ✅ Redirection en cours...");
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Responsive Mobile - Masquer le tableau, afficher les cartes */
...
<body>
  ...
  // Cette section dupliquée a été supprimée - voir ligne 43 pour la définition active

  // ============ ORDER REFUSAL (PUBLIC LINK) - DOUBLON SUPPRIMÉ ============
  // ✅ La route /refuse/:orderId est maintenant définie EN PREMIER (ligne 221)
  // pour éviter l'interception par le middleware Vite/Static
  // Cette section dupliquée a été supprimée - voir ligne 221 pour la définition active
}

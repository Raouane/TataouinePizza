...
export default function DeliveryFormStep2() {
  ...
  const handleContinue = () => {
    ...
    if (!name.trim() || name.length < 2) {
      alert(t('delivery.step2.error.name'));
      return;
    }
    if (!phone.trim() || phone.length < 8) {
      alert(t('delivery.step2.error.phone'));
      return;
    }

    // Récupérer les données de l'étape précédente
    const previousData = localStorage.getItem("delivery_form_data");
    const formData = previousData ? JSON.parse(previousData) : {};
    ...
    setLocation(`/delivery-form-step3?mode=${modeId}`);
  };
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setLocation(`/delivery-form?mode=${modeId}`)}
              className="h-10 w-10"
            >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500">{t('delivery.step', { step: '2' })}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-6 max-w-md">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{t('delivery.step', { step: '2' })}</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            {t('delivery.step2.info.title')}
          </h2>

          <div className="space-y-4">
            {/* Nom */}
            <div className="relative">
              <User className={`absolute top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5 ${isRtl ? 'right-3' : 'left-3'}`} />
              <Input
                type="text"
                placeholder={t('delivery.step2.name.placeholder')}
                value={name}
            ...
            {/* Téléphone */}
            <div className="relative">
              <Phone className={`absolute top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5 ${isRtl ? 'right-3' : 'left-3'}`} />
              <Input
                type="tel"
                placeholder={t('delivery.step2.phone.placeholder')}
                value={phone}
